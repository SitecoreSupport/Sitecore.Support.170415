...
function Initialization() {
  ...
  //Sitecore.Support.170415-1 +++
  function contains(arr, filterType) {
    for (var i = 0; i < arr.length; i++) {
      if (arr[i].type === filterType) {
        return true;
      }
    }
    return false;
  ...
  //Sitecore.Support.170415-1 ---
    ...
    function parseSearchForQuery() {
      ...
      //Sitecore.Support.170415-1 +++
      if (!contains(query, 'location')) {
        var locationFilter = { operation: 'must', type: 'location', value: getParameterByName("id") };
        query.push(locationFilter);
      }
      //Sitecore.Support.170415-1 ---
